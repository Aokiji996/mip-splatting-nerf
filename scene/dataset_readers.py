...
def readMultiScaleSocket(data, white_background,split, only_highres=False, ndown=1):
    cam_infos = []
    
    print("read split:", split)
    meta = data[split]
    meta = {k: meta[k] if k=='img' else np.array(meta[k]) for k in meta}
    
    # should now have ['pix2cam', 'cam2world', 'width', 'height'] in self.meta
    for idx, image in enumerate(meta['img']):
        if only_highres and not idx % ndown == 0:
            continue
        # NeRF 'transform_matrix' is a camera-to-world transform
        c2w = meta["cam2world"][idx]
        # change from OpenGL/Blender camera axes (Y up, Z back) to COLMAP (Y down, Z forward)
        ...
        im_data = np.array(cv2.cvtColor(image, cv2.COLOR_BGR2RGBA))
        ...
        cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, width=image.size[0], height=image.size[1], image_name='', image_path=''))
    return cam_infos


def readMultiScaleNerfSyntheticInfoSocket(datas, white_background, eval, load_allres=False, ndown=1):
    print("Reading train from metadata.json")
    train_cam_infos = readMultiScaleSocket(datas, white_background, "train", only_highres=(not load_allres), ndown=ndown)
    print("number of training images:", len(train_cam_infos))
    print("Reading test from metadata.json")
    test_cam_infos = readMultiScaleSocket(datas, white_background, "test", only_highres=False, ndown=ndown)
    print("number of testing images:", len(test_cam_infos))
    if not eval:
        print("adding test cameras to training")
        train_cam_infos.extend(test_cam_infos)
...
sceneLoadTypeCallbacks = {
    ...
    "Multi-scale-Socket": readMultiScaleNerfSyntheticInfoSocket,
}