...
class GaussianModel:
    ...
    def save_ply(self, path):
        ...
        means3D = xyz
        log_scales = scale
        unnorm_rotations = rotation
        # compute rgb color
        # Spherical harmonic constant
        C0 = 0.28209479177387814
        rgb_colors = f_dc * C0 + 0.5
        logit_opacities = opacities
        timestep = torch.zeros(0).cuda().float()
        # time_idx = 0
        mean_points_per_frame = means3D.shape[0] // 2000
        more_points = means3D.shape[0] - mean_points_per_frame * 2000
        for time_idx in range(2000):
            new_timestep = time_idx * torch.ones(mean_points_per_frame, device="cuda").float()
            timestep = torch.cat((timestep, new_timestep), dim=0)
        new_timestep = 1999 * torch.ones(more_points, device = "cuda").float()
        timestep = torch.cat((timestep, new_timestep), dim=0)
        # for i in range(means3D.shape[0]):
        #     for j in range(2000):
        #         if i < (j+1)*mean_points_per_frame:
        #             new_timestep = j * torch.ones()
        params = {
            'means3D': means3D,
        ...
        for k, v in params.items():
            if isinstance(v, torch.Tensor):
                ...
                res[k] = v
        os.makedirs(path[:-16], exist_ok=True)
        print(f"Saving parameters to: {path[:-16]}")
        save_path = os.path.join(path[:-16], "params.npz")
